...
function useMastheadCinema(root: React.RefObject<HTMLDivElement | null>, statsRef: React.RefObject<HTMLSpanElement | null>, ready: boolean) {
...
export default function PublicProfile() {
  ...
  <MotionSection id="featured" className="section container-shell"><SectionHead kicker="02 · Hall of fame" title={<>The games that <span className="text-gradient">stayed</span></>} body="The permanent collection. Open a card to turn it over and read the complete field notes."/><div className="grid grid-cols-2 gap-3 sm:gap-5 sm:grid-cols-2 lg:grid-cols-3">{featured.map((g,i)=><Reveal key={g.id} delay={(i%3)*0.08}><GameCard game={g} onOpen={()=>open(g)}/></Reveal>)}</div></MotionSection>
