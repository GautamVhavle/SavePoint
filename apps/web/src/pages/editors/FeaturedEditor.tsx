        ...
        return <li key={entry.id} className="rounded-2xl border border-white/10 p-3">
          ...
          {draft.featured && <div className="mt-3 grid gap-3 border-t border-white/10 pt-3 sm:grid-cols-[120px_1fr]">
            <label><span className="label">ORDER</span>
              <input className="field" type="number" inputMode="numeric" min="0" max="10000" aria-describedby="featured-order-hint" value={draft.order ?? ''} onChange={event => update(entry.id, { order: event.target.value === '' ? null : Number(event.target.value) })}/>
              <span id="featured-order-hint" className="sr-only">Lower numbers appear higher on your public Hall of Fame.</span>
            </label>
